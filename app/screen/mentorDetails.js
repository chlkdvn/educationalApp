...
const MentorDetail = () => {
    ...
    const fetchEducatorData = async () => {
      try {
        ...
        const response = await fetch(
          `${backendUrl}/api/user/getEducatorOverview/${educatorId}`,
          {
  ...
  useEffect(() => {
    ...
    const fetchFollowData = async () => {
      try {
        ...
        const countRes = await axios.get(
          ...
          { headers: { Authorization: `Bearer ${token}` } } // token optional here but safe
        );
    ...
    fetchFollowData();
  }, [data, educatorId]); // Runs as soon as main data is available
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      ...
      // On success: optionally refetch count to be 100% sure (safe)
      // But optimistic is usually fine
    } catch (err) {
                ...
                >
                  ...
                  <View style={styles.courseDetails}>
                    <View style={styles.courseHeader}>
                      ...
                      <Text style={styles.bookmarkIcon}>Bookmark</Text>
                    </View>
